...
function clean(options, callback) {
  ...
  eachPackage(limit, function iterator(handler, cb) {
    ...
    var opts = {force: true};
    if (options.dest) {
      console.log('deleting', handler.destDir);
      del(handler.destDir).then(function() {
        cb();
      });
    } else if (options.src) {
      del(path.join(handler.srcDir, 'node_modules')).then(function() {
        cb();
      });
    }
  }, callback);
...
gulp.task('js', ['find-packages'], function(callback) {
  var excludeFiles = [
    '!package.json',
    '!README.md',
    '!example.js',
